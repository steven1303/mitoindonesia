<section class="content-header">
    <h1>
        PO Internal New @if($po_internal->po_status == 1 ) Draft @endif @if($po_internal->po_status == 2 ) Open @endif
    </h1>
    <ol class="breadcrumb">
...
<section class="content">
                ...
                <div class="box-body">
                    <form role="form" id="SpbdForm" method="POST">
                        ...
                        <div class="box-footer">
                            ...
                            @endif
                            <button class="btn btn-secondary" type="button" onclick="ajaxLoad('{{route('local.po_internal.new.index')}}')">Save</button>
                        </div>
                    </form>
                </div>
            </div>
    ...
    <div class="row">
        <div class="col-xs-12">
            <div class="box">
                <div class="box-header with-border">
                    <h3 class="box-title">PO Internal New Detail</h3><br/><br/>
                    @if($po_internal->po_status == 1 || $po_internal->po_status == 2 )
...
<script type="text/javascript">
    ...
    .DataTable({
        ...
        "serverSide"	: true,
        responsive      : true,
        "ajax": "{{route('local.record.po_internal_detail.new', $po_internal->id ) }}",
        "columns": [
    ...
    $(function(){
	    ...
	    $('#PoInternalDetailForm').validator().on('submit', function (e) {
		    ...
		    if (!e.isDefaultPrevented()){
			    if (save_method == 'add')
			    {
				    url = "{{route('local.po_internal.new.store_detail', $po_internal->id) }}";
				    $('input[name=_method]').val('POST');
			    } else {
				    url = "{{ url('po_internal_new_detail') . '/' }}" + id;
    ...
    function editForm(id) {
        ...
        $('input[name=_method]').val('PATCH');
        $.ajax({
        url: "{{ url('po_internal_new') }}" + '/' + id + "/edit_detail",
        type: "GET",
        dataType: "JSON",
        beforeSend:function(){
            $(document).find('span.error-text').text('');
    ...
    @endcan
    @can('po.internal.new.open', Auth::user())
    function open_po_internal_Form() {
        $.ajax({
            url: "{{route('local.po_internal.new.open.index', $po_internal->id) }}",
            type: "GET",
            dataType: "JSON",
            success: function(data) {
                if(data.stat == "Error")
                ...
                if(data.stat == "Success"){
                    ...
                    ajaxLoad("{{ route('local.po_internal.new.index') }}");
